...
timer_js = '''
        ...
        if(timerContainer) {
            ...
            if(tStyle === "circular_classic") {
                tHTML = `
                <div class="clock-outer" style="width:340px; height:340px; border-radius:50%; background:#0a0a0a; border:12px solid #1a1a1a; box-shadow:0 0 60px ${mainColor}80; position:relative; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                  ...
                  <div class="clock-inner" style="width:260px; height:260px; border-radius:50%; border:6px solid ${mainColor}40; display:flex; align-items:center; justify-content:center; flex-direction:column; position:relative;">
                    ...
                    <span id="timer-num" style="position:relative; font-size:110px; font-weight:900; color:${mainColor}; font-family:monospace!important; z-index:2; line-height:1;">00:05</span>
                  </div>
                </div>`;
            } else if (tStyle === "horizontal_bar") {
                tHTML = `
                <div style="display:flex; flex-direction:column; align-items:center; gap: 20px; width: 400px; padding: 20px; background:#111; border-radius:40px; border:4px solid ${mainColor}40; box-shadow:0 0 40px ${mainColor}40;">
                   <span id="timer-num" style="font-size:120px;font-weight:900;color:${mainColor};font-family:monospace!important;letter-spacing:10px; line-height:1; text-shadow: 0 0 30px ${mainColor}80;">00:05</span>
                   <div style="width:100%; height:40px; background:#000; border-radius:20px; overflow:hidden; position:relative; box-shadow: inset 0 0 20px rgba(0,0,0,0.8);">
                       <div id="clock-bar" style="width:100%; height:100%; background:${mainColor}; box-shadow: 0 0 20px ${mainColor}; transition: width 0.1s linear;"></div>
                   </div>
                </div>`;
            } else if (tStyle === "square_outline") {
                tHTML = `
                <div style="position:relative; width:340px; height:340px; display:flex; align-items:center; justify-content:center; transform: rotate(45deg); background:#0a0a0a; border-radius: 40px; box-shadow:0 0 60px ${mainColor}40;">
                    <svg width="340" height="340" style="position:absolute; top:0; left:0; filter: drop-shadow(0 0 20px ${mainColor});">
                       <rect id="clock-ring" x="20" y="20" width="300" height="300" rx="30" stroke="${mainColor}" stroke-width="16" fill="none" stroke-dasharray="1200" stroke-dashoffset="0" style="transition: stroke-dashoffset 0.1s linear;" />
                    </svg>
                    <div style="transform: rotate(-45deg); text-align:center; display:flex; flex-direction:column; align-items:center; justify-content:center;">
                        ...
                        <span id="timer-num" style="font-size:110px;font-weight:900;color:${mainColor}; line-height:1; font-family:monospace!important;">00:05</span>
                    </div>
                </div>`;
            } else if (tStyle === "liquid_drain") {
                tHTML = `
                <div style="display:flex; flex-direction:column; align-items:center;">
                    <div style="width:80px; height:20px; background:${mainColor}80; border-radius:20px 20px 0 0; box-shadow: 0 -10px 20px ${mainColor}40;"></div>
                    <div style="width: 200px; height: 320px; border: 12px solid #222; border-radius: 40px; position:relative; overflow:hidden; display:flex; align-items:center; justify-content:center; background:#000; box-shadow: 0 0 50px ${mainColor}40;">
                        <div id="clock-fill" style="position:absolute; bottom:0; left:0; width:100%; height:100%; background:${mainColor}; box-shadow: 0 0 40px ${mainColor}; transition: height 0.1s linear;"></div>
...
'''
html = html.replace('decContainer.innerHTML = frameHTML;', timer_js)
